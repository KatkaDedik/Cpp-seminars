...
void grep(std::string pattern, std::istream & input, std::ostream & output)
{
	while (!input.eof())
	{
		std::string line;
...
void cut(char delim, int col, std::istream & input, std::ostream & output) 
{
	for (;;)
	{
		std::string line;
		...
		for (int i = 0; i <= col; i++) 
		{
			...
			if(!std::getline(ss, substring, delim))
			{
				output << "\n";
			}
			else if (i == col)
