...
#include <memory>
 ...
 /* To make testing easier, we declare a global counter for matrices.
...
class solid {
	double x, y, z;
	std::unique_ptr<transform_matrix> matrix = nullptr;

public:
	solid(double a_x, double a_y, double a_z) : x(a_x), y(a_y), z(a_z) {}

	double pos_x() const { return x; }
	double pos_y() const { return y; }
	double pos_z() const { return z; }
	...
	void transform_set(int r, int c, double v) {
		if (matrix.get() == nullptr) {
			matrix.reset(new transform_matrix());
		}
		matrix->matrix[r][c] = v;
	}
};
...
int main()
{
	...
	assert(sizeof(solid) <= 3 * sizeof(double) + sizeof(void *));
	assert(sizeof(transform_matrix) == 9 * sizeof(double));
