...
json_integer integer_parse(const std::string &s, size_t &index) {
	...
	while (number.find(s[index]) != std::string::npos) {
		index++;
	}
...
json_object object_parse(const std::string &s, size_t &index) {
	...
	do {
		...
		{
			throw json_error("key is already in object!");
		}
...
json_ptr json_parse_recursive(const std::string &s, size_t &index) {
	...
	if (nonzero.find(s[index]) != std::string::npos) {
		...
		case '0':
			...
			if (nonzero.find(s[index]) != std::string::npos) {
				throw json_error("invalid json_integer");
			}
...
json_ptr json_parse(const std::string &s) {
	size_t index = 0;
	json_ptr ret_ptr = json_parse_recursive(s, index);
