...
public:
	...
	virtual bool betray(bool lastTurnDecision) = 0;
};
...
public:
	...
	bool betray(bool lastTurnDecision) {
		strategy.push_back(lastTurnDecision);
		return true;
	}
};
...
int play(prisoner &a, prisoner &b, int rounds) 
{
	...
	{
		bool tmp = aChoice;
		aChoice = a.betray(bChoice);
		bChoice = b.betray(tmp);
