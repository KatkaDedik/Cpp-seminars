...
public:
	int value;
	virtual bool eval(const input& in) const = 0;
}; /* ref: 6 lines */
...
public:
	...
	operation(unsigned uu, const node& aa, const node& bb) :
		u(uu),
		a(aa),
		b(bb)
	{}

	bool eval(const input& in) const {
		bool aValue = a.eval(in);
		bool bValue = b.eval(in);
		int tmp = aValue + 2 * bValue;
		return((u >> tmp) & 0b0001 == 0b0001);
	}
...
int main()
{
	...
	operation zero(op_xor, var_1, var_1), one(op_nor, zero, zero);
