...
std::vector<uint32_t> add(const integer_number &first,
                          const integer_number &second) {
  size_t index = 0;
...
std::vector<uint32_t> subtract(const integer_number &first,
                               const integer_number &second,
                               size_t offset = 0) {
  size_t index = 0;
  std::vector<uint32_t> ret;
...
const std::array<
    std::array<std::function<integer_number(const integer_number &,
                                            const integer_number &)>,
               2>,
    2>
    integer_number::sign_table = {{{{table_function_pp, table_function_pn}},
...
integer_number integer_number::table_function_nn(const integer_number &first,
                                                 const integer_number &second) {
  // both negative, -first -second = -(first + second)
  return integer_number(add(first, second), true);
}

integer_number integer_number::table_function_np(const integer_number &first,
                                                 const integer_number &second) {
  // first negativ, second positive, -first +second
  if (first.abs_cmp(second) == 1) {
    return integer_number(subtract(first, second),
                          true); // |first| > |second| -> -(first - second)
  }
  return integer_number(subtract(second, first),
                        false); // |first| <= |second| -> +(second - first)
}

integer_number integer_number::table_function_pn(const integer_number &first,
                                                 const integer_number &second) {
...
integer_number integer_number::table_function_pp(const integer_number &first,
                                                 const integer_number &second) {
...
integer_number integer_number::devide(const uint32_t &denominator,
                                      int offset) const {
...
std::tuple<integer_number, integer_number>
integer_number::devide_mod(const integer_number &denominator) const {
  integer_number numerator = *this;
  uint64_t numerator_tmp = 0;
  const uint64_t denominator_tmp = denominator.get(denominator.size() - 1);
  integer_number numerator_upper_elements =
      numerator.get_elements(numerator.size(), denominator.size() - 1);
  std::vector<uint32_t> ret;

  for (int i = static_cast<int>(numerator.size()) - 1;
       i >= static_cast<int>(denominator.size()) - 1; i--) {

    numerator_upper_elements.push_forward(get(i - (denominator.size() - 1)));
    uint64_t tmp = numerator_upper_elements.get(denominator.size());
    numerator_tmp =
        (tmp << 32) | numerator_upper_elements.get(denominator.size() - 1);
    ...
    integer_number upper_remainder =
        numerator_upper_elements - upper_x_denominator;
    ...
    if (!upper_remainder.sgn()) {
      ...
      integer_number lower_remainder =
          numerator_upper_elements - lower_x_denominator;
      ...
      while (lower_remainder.abs_cmp(denominator) != -1) {
        ...
        if (numerator_tmp < denominator_tmp) {
          numerator_tmp = numerator_tmp << 32 |
                          lower_remainder.get(lower_remainder.size() - 2);
        }

        upper_guess = numerator_tmp / denominator_tmp;

        upper_remainder =
            lower_remainder - denominator.multiply(upper_guess, 0);
        ...
        if (!upper_remainder.sgn()) {
          ...
          lower_remainder =
              lower_remainder - denominator.multiply(lower_guess2, 0);
...
int integer_number::cmp(const integer_number &right) const {
  ...
  if (data.size() == 1 && right.size() == 1 && data[0] == 0 &&
      right.get(0) == 0) {
    return 0;
  }
...
void integer_number::change_sign() { sign = !sign; }

integer_number integer_number::get_elements(size_t offset, size_t count) const {
  /*if (offset + count >= data.size()) {
          return integer_number(0);
  }*/
  return integer_number(std::vector<uint32_t>(data.begin() + offset - count,
                                              data.begin() + offset),
                        false);
}
...
number number::operator+(const number &num) const {
  ...
  number ret((numerator * num.get_denominator()) +
                 (num.get_numerator() * denominator),
...
number number::operator-(const number &num) const {
  ...
  integer_number new_numerator =
      (numerator * num.get_denominator()) - (num.get_numerator() * denominator);
...
number number::operator*(const number &num) const {
  ...
  number ret(numerator * num.get_numerator(),
             denominator * num.get_denominator());
  return ret.simplify();
}
...
bool number::sgn() const { return (numerator.sgn() ^ denominator.sgn()); }
int number::cmp(const number &num) const {
  ...
  return (numerator * num.get_denominator())
      .cmp(num.get_numerator() * denominator);
}
