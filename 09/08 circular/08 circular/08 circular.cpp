...
{
	std::unique_ptr< circular_node > next;
	circular_node * prev;
	T value;
...
class circular 
{
	...
	std::unique_ptr < circular_node<T> > first;
	circular_node<T> * last;
...
public:
	...
	{
		if (!empty()) {
			last->next = std::make_unique< circular_node< T > >();
			last->next.get()->prev = last;
			last = last->next.get();
		}
		else
		{
			first = std::make_unique< circular_node< T > >();
			last = first.get();
		}
		last->value = value;
		first.get()->prev = last;
	...
	void rotate() {
		if (!empty())
		{
			last->next = std::move(first);
			first = std::move(last->prev->next);
			last = last->prev;
		}
	}

	void pop() {
		if (first.get()->next == nullptr) {
			first.release();
		}
		if (!empty())
		{
